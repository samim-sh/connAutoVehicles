...
def k_monteCarlo(montecarlo_iteration):
    ...
    for iterate in range(montecarlo_iteration):
        ...
        df_car_queue.loc[(df_car_queue.queue == 'h') & (
                df_car_queue.queue_shift_neg_1 == 'c'), 'h_mn_kMonteCarlo'] = h_hc_sample  # hc
        df_car_queue.loc[(df_car_queue.queue == 'c') & (
                df_car_queue.queue_shift_neg_1 == 'h'), 'h_mn_kMonteCarlo'] = h_ch_sample  # ch
        df_car_queue.loc[(df_car_queue.queue == 'h') & (
                df_car_queue.queue_shift_neg_1 == 'h'), 'h_mn_kMonteCarlo'] = h_hh_sample  # hh
...
if __name__ == '__main__':
    ...
    for P_c in np.round(np.arange(0.1, 1, .1).tolist(), 2)[7:8]:
        ...
        for each_sheet in xl_dist_headway.sheet_names[:]:
            ...
            for indx_row, each_hour in df_dist_table_headway[:].iterrows():
                ...
                df_speed_hour = df_speed[(df_speed.DateTime.dt.time > start_peak) &
                ...
                n_cc, n_ch, n_hh, n_hc = 0, 0, 0, 0
                while (n_cc + n_ch) == 0 or (n_hh + n_hc) == 0:
                ...
                with Pool(4) as pool:
                    lst_k = pool.map(k_monteCarlo, [int(montecarlo_iteration / 4) for x in range(4)])
                if k_mixed_jam > sum(lst_k) / montecarlo_iteration > k_mixed_critical:
                    # monteCarlo for C-mixed
                    with Pool(4) as pool:
                        lst_t_c_v_c = pool.map(t_monteCarlo, [int(montecarlo_iteration / 4) for x in range(4)])
                    lst_t,lst_c,lst_v_c = [i[0] for i in lst_t_c_v_c],[i[1] for i in lst_t_c_v_c],[i[2] for i in lst_t_c_v_c]
                    lst_summary_table[day_date][lane_id][start_peak] = [sum(lst_t)/montecarlo_iteration, sum(lst_c)/montecarlo_iteration, sum(lst_v_c)/montecarlo_iteration]
                else:
