...
def func_to_multi(eee):
	...
	df_speed = pd.read_excel('datasets/speed_chosen.xlsx',sheet_name=f'{day_date}_laneId_{lane_id}', usecols=['DateTime', 'timeHeadway', 'Speed'])
		# xl_speed.parse(f'{day_date}_laneId_{lane_id}', usecols=['DateTime', 'timeHeadway', 'Speed'])
	...
	df_car_queue.loc[(df_car_queue.queue == 'c') & (df_car_queue.queue_shift_neg_1 == 'h'), 'P_mn'] = P_h * (
				1 - P_I)  # ch
	df_car_queue.loc[(df_car_queue.queue == 'c') & (df_car_queue.queue_shift_neg_1 == 'c'), 'P_mn'] = 1 - P_h * (
				1 - P_I)  # cc
	df_car_queue.loc[(df_car_queue.queue == 'h') & (df_car_queue.queue_shift_neg_1 == 'c'), 'P_mn'] = P_c * (
				1 - P_I)  # hc
	...
	if k_mixed_jam > lst_k_results[-1] > k_mixed_critical:
		...
		return start_peak,[lst_t_results[-1], lst_c_results[-1], lst_v_c_results[-1]]
	else:
		# print(f'{P_c} {day_date} {start_peak} {lane_id}', k_mixed_critical, lst_k_results[-1], k_mixed_jam)
		return start_peak,[None, None, None]
