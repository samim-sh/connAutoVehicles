...
from sklearn.linear_model import SGDRegressor, BayesianRidge, LinearRegression, Ridge, Lasso, TweedieRegressor, \
    PassiveAggressiveRegressor, RANSACRegressor, TheilSenRegressor, HuberRegressor, QuantileRegressor
    ...
    for P_c in np.round(np.arange(0.1, 1, .1).tolist(), 2)[6:7]:
        ...
        for day in lst_summary_table:
            ...
            for lan in lst_summary_table[day]:
                tups = [i for b in
                        [[(day, x, 't'), (day, x, 'c'), (day, x, 'v_c')] for x in lst_summary_table[day][lan].keys()]
                        for i in b]
                colums = pd.MultiIndex.from_tuples(tups, names=['date level 1', 'hour level 2', 'var level 3'])
                df_summary_table = pd.DataFrame([[i for b in lst_summary_table[day][lan].values() for i in b]],
                                                columns=colums, index=[lan])
